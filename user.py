import sys
from PyQt5.QtWidgets import QMainWindow, QApplication, QPushButton, QTableWidgetItem, QMessageBox, QMenu, QInputDialog, QLineEdit
...
class User(QMainWindow):
  ...
  def simpan(self):
    try:
      ...
      if idUser != '' and nDepan != '' and tipeUser != '' and username != '' and password != '':
        ...
        # menampilkan data user pada tabel dengan memanggil fungsi tampilDataUser
        self.tampilDataUser()
      else:
  ...
  # fungsi mengubah passwor
  def ubahPassword(self, idUser, passBaru):
    try:
      if idUser != '' and passBaru != '':
        # konek ke mysql
        con = mc.connect(
            host='localhost',
            user='root',
  ...
  # fungsi membuat context menu pada tabel
  def eventFilter(self, source, event):
    try:
      if event.type() == QEvent.ContextMenu and source is self.tblUser.viewport():
        menu = QMenu()
        menu.addAction(QIcon('ICON/change_password.png'), 'Ubah Password')
        
        if menu.exec_(event.globalPos()):
          items = self.tblUser.selectedItems()
          idUser = items[0].text()
          
          # menampilkan kotak teks untuk memasukkan password baru
          passBaru, ok=QInputDialog.getText(self, 'Data Perpustakaan', 'Password Baru', QLineEdit.Password, '')
          
          if ok:
            self.ubahPassword(idUser, passBaru)
      
      return super().eventFilter(source, event)
    
    except:
      self.tampilPesan('Terjadi kesalahan saat membuat kontek menu')
